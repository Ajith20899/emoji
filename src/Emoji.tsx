...
import GifList from '@fixtures/emoji/gif.json';
...
export function EmojiPicker() {
    ...
    const addEmojiIntoChat = (html, textarea) => {
        var sel, range;
        if (window.getSelection) {
            // IE9 and non-IE
            sel = window.getSelection();
    ...
    const emojiGifHandler = (emoji: string) => {
        var html;
        var textarea = document.getElementById('textarea');
        let getGif = GifList[`${emoji}`];
        html = `<img
                    class="emoji"
                    src="${getGif}"
                    draggable="false"
                ...
                />&nbsp;`;
        textarea?.focus();
        addEmojiIntoChat(html, textarea);
    }

    const emojiImageHandler = (emoji: string) => {
        var html;
        var textarea = document.getElementById('textarea');
    ...
    const emojiHandler = (emoji: string, type?: String) => {
        if (!emoji) return;
        try {
            if (type === "gif") {
                emojiGifHandler(emoji);
            } else {
                emojiImageHandler(emoji);
            }
        } catch (err) {
            console.log('err', err);
        }
